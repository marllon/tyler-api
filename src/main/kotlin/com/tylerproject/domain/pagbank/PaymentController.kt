...
class PaymentController @Autowired constructor(private val pagBankProvider: PagBankProvider) {
    ...
    fun createCheckout(
        ...
        return try {
            ...
            val pagBankRequest = buildMap<String, Any> {
                ...
                val customer = buildMap<String, String> {
                    request.customerName?.let { put("name", it) }
                    request.customerEmail?.let { put("email", it) }
                    request.customerPhone?.let { put("phone", it) }
                    request.customerDocument?.let { put("tax_id", it) }
                }
    ...
    @ApiResponses(
        value = [
            ...
            ApiResponse(responseCode = "500", description = "❌ Erro ao consultar status")
        ]
    )
