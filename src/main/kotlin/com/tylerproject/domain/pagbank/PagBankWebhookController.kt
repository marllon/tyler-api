...
class PagBankWebhookController(
        ...
        fun handlePagBankWebhook(
                ...
                return try {
                        logger.info(
                                "📨 Received PagBank webhook - notificationId: ${payload.notificationId}"
                        )
                        logger.debug("Webhook payload: $payload")

                        // Identificar tipo de pagamento pelo reference_id
                        val referenceId =
                                payload.referenceId ?: payload.charges?.firstOrNull()?.referenceId

                        val result =
                                when {
                                        referenceId?.startsWith("ORD-") == true -> {
                                                logger.info(
                                                        "🛒 Processing order payment webhook - orderNumber: $referenceId"
                                                )
        ...
        private fun processOrderWebhook(
                ...
                return try {
                        if (status.uppercase() == "PAID" && paymentId != null) {
                                // OrderService.processOrderPayment usa paymentId para encontrar e
                                // processar o pedido
