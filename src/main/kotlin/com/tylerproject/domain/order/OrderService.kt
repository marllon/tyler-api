...
class OrderServiceImpl(
    ...
    override fun createOrder(
        ...
        val orderDetails =
                ProductOrderDetails(
                        ...
                        subtotal = subtotal,
                        shippingMethod =
                                when (request.shippingMethod) {
                                    ShippingMethod.COLLECT_ON_DELIVERY ->
                                            com.tylerproject.domain.transaction.ShippingMethod
                                                    .COLLECT_ON_DELIVERY
                                    ShippingMethod.SEDEX ->
                                            com.tylerproject.domain.transaction.ShippingMethod.SEDEX
                                    ShippingMethod.PAC ->
                                            com.tylerproject.domain.transaction.ShippingMethod.PAC
                        ...
                        shippingAddress =
                                com.tylerproject.domain.transaction.ShippingAddress(
    ...
    override fun getOrderById(userId: String, orderId: String): OrderResponse? {
        ...
        if (order.userId != userId) {
            logger.warn(
                    "User $userId attempted to access order $orderId belonging to ${order.userId}"
            )
            return null
        }
    ...
    override fun getOrderDetails(userId: String, orderId: String): OrderDetailsResponse? {
        ...
        val paymentDetails =
                transaction?.payment?.let { payment ->
                    PaymentDetailsResponse(
                    ...
                    TrackingResponse(
                            ...
                            events =
                                    order.trackingEvents.map {
                                        TrackingEventResponse.fromEntity(it)
                                    }
                    )
                } else {
                    null
        ...
        try {
            ...
            for (item in order.items) {
                ...
                if (product == null || productStock < item.quantity) {
                    logger.error(
                            "Insufficient stock for product ${item.productId} in order ${order.orderNumber}"
                    )
    ...
    private fun generateOrderNumber(): String {
        ...
        val sequence =
                (now.atZone(java.time.ZoneId.systemDefault()).toEpochSecond() % 10000)
                        .toString()
                        .padStart(4, '0')
