...
class TransactionServiceImpl(
    ...
    override fun createTransaction(request: CreateTransactionRequest): Transaction = runBlocking {
        ...
        val transactionWithPayment =
                if (request.paymentMethod == TransactionPaymentMethod.PIX) {
                    createPixPayment(savedTransaction, request)
                } else {
                    // TODO: Implementar outros métodos de pagamento
                    throw UnsupportedOperationException(
                            "Método de pagamento ${request.paymentMethod} não implementado"
                    )
                }
                        ...
                        "payment" to
                                mapOf(
                                        ...
                                        "qrCodeImageBase64" to
                                                transactionWithPayment.payment!!.qrCodeImageBase64,
        ...
        logger.info(
                "Transaction updated with payment info: paymentId=${transactionWithPayment.payment?.paymentId}"
        )
    ...
    override fun updatePaymentStatus(
        ...
        val paidAtValue =
                paidAt ?: if (status == TransactionStatus.PAID) now else transaction.paidAt
        if (paidAtValue != null) {
    ...
    private suspend fun createPixPayment(
        ...
        val pagBankRequest =
                mapOf(
                        ...
                        "reference_id" to
                                transaction.id, // ✅ CRÍTICO: usar transaction.id para vincular
                        "payer" to
                                mapOf(
                                        ...
                                        "document" to
                                                (transaction.customer.document
                                                        ?: "12345678909") // CPF padrão para teste
                                ),
        ...
        return transaction.copy(payment = paymentInfo, updatedAt = getCurrentTimestamp())
    }
...
data class CreateTransactionRequest(
    ...
    fun validate() {
        ...
        when (type) {
            TransactionType.SIMPLE_DONATION ->
                    require(simpleDonationDetails != null) {
                        "simpleDonationDetails required for SIMPLE_DONATION"
                    }
            TransactionType.GOAL_CONTRIBUTION ->
                    require(goalContributionDetails != null) {
                        "goalContributionDetails required for GOAL_CONTRIBUTION"
                    }
            TransactionType.RAFFLE_PURCHASE ->
                    require(rafflePurchaseDetails != null) {
                        "rafflePurchaseDetails required for RAFFLE_PURCHASE"
                    }
            TransactionType.PRODUCT_ORDER ->
                    require(productOrderDetails != null) {
