...
class FirestoreTransactionRepository(private val firestore: Firestore) : TransactionRepository {
    ...
    override fun findByPaymentId(paymentId: String): Transaction? {
        return try {
            val query = collection.whereEqualTo("payment.paymentId", paymentId).limit(1)

            val snapshot = query.get().get()
            val dto =
                    snapshot.documents.firstOrNull()?.toObject(TransactionFirestoreDto::class.java)
            dto?.toDomain()
        } catch (e: Exception) {
    ...
    override fun findByUserId(
        ...
        return try {
            ...
            val nextCursor =
                    if (transactions.isNotEmpty() && limit != null && transactions.size >= limit) {
