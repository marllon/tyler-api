    ...
    fun toDomain(): Transaction {
        return Transaction(
                ...
                simpleDonationDetails =
                        simpleDonationDetails?.let { mapToSimpleDonationDetails(it) },
                goalContributionDetails =
                        goalContributionDetails?.let { mapToGoalContributionDetails(it) },
                rafflePurchaseDetails =
                        rafflePurchaseDetails?.let { mapToRafflePurchaseDetails(it) },
    ...
    companion object {
        fun fromDomain(transaction: Transaction): TransactionFirestoreDto {
            return TransactionFirestoreDto(
                    ...
                    simpleDonationDetails =
                            transaction.simpleDonationDetails?.let {
                                simpleDonationDetailsToMap(it) as Map<String, Any>?
                            },
                    goalContributionDetails =
                            transaction.goalContributionDetails?.let {
                                goalContributionDetailsToMap(it) as Map<String, Any>?
                            },
                    rafflePurchaseDetails =
                            transaction.rafflePurchaseDetails?.let {
                                rafflePurchaseDetailsToMap(it) as Map<String, Any>?
                            },
                    productOrderDetails =
                            transaction.productOrderDetails?.let {
                                productOrderDetailsToMap(it) as Map<String, Any>?
                            },
        ...
        private fun mapToRafflePurchaseDetails(map: Map<String, Any>): RafflePurchaseDetails {
            ...
            return RafflePurchaseDetails(
                    ...
                    ticketCount = (map["ticketCount"] as? Number)?.toInt()
                                    ?: ticketNumbersData.size,
                    isAnonymous = (map["isAnonymous"] as? Boolean) ?: false
            )
        }
        ...
        private fun simpleDonationDetailsToMap(details: SimpleDonationDetails): Map<String, Any?> {
            return mapOf("message" to details.message, "isAnonymous" to details.isAnonymous)
        }

        private fun goalContributionDetailsToMap(
                details: GoalContributionDetails
        ): Map<String, Any?> {
            return mapOf(
