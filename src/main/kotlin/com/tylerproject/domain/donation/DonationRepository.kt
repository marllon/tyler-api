...
@Repository
class DonationRepository(private val firestore: Firestore) {
    private val collection = "donations"

    fun save(donation: Donation): Donation {
        val docRef =
                if (donation.id.isBlank()) {
    ...
    fun findByTargetId(targetId: String, donationType: DonationType? = null): List<Donation> {
        var query: Query = firestore.collection(collection).whereEqualTo("targetId", targetId)

        donationType?.let { query = query.whereEqualTo("donationType", it.name) }
    ...
    fun sumAmountByTargetId(targetId: String, status: DonationStatus): Double {
        ...
        return donations.filter { it.status == status }.sumOf { it.amount }
    }
}
