...
class DonationController(private val donationService: DonationService) {
        ...
        @PostMapping("/simple")
        @Operation(
                summary = "Criar doação simples",
                description =
                        "Cria uma doação simples sem vínculo com metas, rifas ou produtos. Retorna QR Code PIX para pagamento."
        )
        @ApiResponses(
                value =
                        [
                                ApiResponse(
                                        responseCode = "200",
                                        description = "Doação criada e QR Code gerado com sucesso"
                                ),
                                ApiResponse(
                                        responseCode = "400",
                                        description =
                                                "Dados inválidos (valor mínimo, email inválido, dados obrigatórios faltando)"
                                ),
                                ApiResponse(
                                        responseCode = "500",
        ...
        fun createSimpleDonation(
                @Valid @RequestBody request: SimpleDonationRequest
        ): ResponseEntity<Any> {
                return try {
                        logger.info(
                                "Creating simple donation - amount: ${request.amount}, anonymous: ${request.anonymous}"
                        )
                        val response = donationService.createSimpleDonation(request)
                        ResponseEntity.ok(response)
                } catch (e: IllegalArgumentException) {
                        logger.error("Validation error creating simple donation: ${e.message}")
                        ResponseEntity.badRequest().body(mapOf("error" to e.message))
                } catch (e: IllegalStateException) {
                        logger.error("Error generating PIX: ${e.message}")
                        ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                                .body(mapOf("error" to e.message))
                } catch (e: Exception) {
                        logger.error("Unexpected error creating simple donation: ${e.message}", e)
                        ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                                .body(mapOf("error" to "Erro ao processar pagamento PIX"))
                }
        }
        ...
        fun listDonations(
                ...
                @Parameter(
                        description =
                                "Filtrar por status (PENDING, PAID, CANCELLED, REFUNDED, FAILED)"
                )
        ...
        @Operation(
                ...
                description =
                        "Lista todas as doações vinculadas a uma meta, rifa ou pedido específico"
        )
        @ApiResponses(
                value =
                        [
        ...
        fun getDonationsByTargetId(
                ...
                return try {
                        ...
                        logger.error(
                                "Error getting donations by targetId $targetId: ${e.message}",
                                e
                        )
                        ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build()
                }
