...
@Tag(
    name = "💰 Donations",
    description = "API de Gerenciamento de Doações - vincula pagamentos do PagBank com Metas/Rifas/Pedidos"
)
class DonationController(
    ...
    @Operation(
        ...
        description = "Cria uma nova doação vinculada a uma meta, rifa ou pedido. Retorna os dados da doação criada."
    )
    @ApiResponses(
        value = [
            ApiResponse(responseCode = "201", description = "Doação criada com sucesso"),
            ApiResponse(responseCode = "400", description = "Dados inválidos"),
            ApiResponse(responseCode = "404", description = "Meta/Rifa/Pedido não encontrado"),
    ...
    fun createDonation(
        ...
        return try {
            logger.info("Creating donation - type: ${request.donationType}, targetId: ${request.targetId}")
    ...
    @Operation(
        ...
        description = "Cria uma cobrança PIX no PagBank e retorna o QR Code para pagamento. Só funciona para doações com método PIX e status PENDING."
    )
    @ApiResponses(
        value = [
            ApiResponse(responseCode = "200", description = "QR Code gerado com sucesso"),
            ApiResponse(responseCode = "400", description = "Doação não está em status válido ou método não é PIX"),
            ApiResponse(responseCode = "404", description = "Doação não encontrada"),
            ApiResponse(responseCode = "500", description = "Erro ao gerar QR Code no PagBank")
        ]
    )
    fun createPixCharge(
        @Parameter(description = "ID único da doação")
    ...
    @Operation(
        ...
        description = "Lista todas as doações com paginação e filtros opcionais por status e tipo"
    )
    @ApiResponses(
        value = [
            ApiResponse(
                ...
                content = [Content(
                    ...
                    schema = Schema(implementation = DonationPageResponse::class)
                )]
            ),
            ApiResponse(responseCode = "500", description = "Erro interno do servidor")
        ]
    ...
    fun listDonations(
        ...
        @Parameter(description = "Filtrar por status (PENDING, PAID, CANCELLED, REFUNDED, FAILED)")
        ...
        return try {
            logger.info("Listing donations - page: $page, pageSize: $pageSize, status: $status, type: $donationType")
            val response = donationService.listDonations(page, pageSize, status, donationType, sortBy, sortDirection)
            ResponseEntity.ok(response)
        } catch (e: Exception) {
    ...
    @Operation(
        ...
        description = "Força o processamento de uma doação paga (adiciona valor à meta/rifa/pedido). Uso administrativo.",
        security = [SecurityRequirement(name = "firebase-auth")]
    )
    @ApiResponses(
        value = [
            ApiResponse(responseCode = "200", description = "Doação processada com sucesso"),
            ApiResponse(responseCode = "400", description = "Doação não está paga ou já foi processada"),
            ApiResponse(responseCode = "404", description = "Doação não encontrada"),
            ApiResponse(responseCode = "500", description = "Erro ao processar doação")
        ]
    )
    fun processDonation(
        @Parameter(description = "ID único da doação")
        ...
        return try {
            ...
            val response = ProcessDonationResponse(
                success = processed,
